...
// [LEGACY / DEPRECATED] 2 API nay thao tac tren bang project_members (co che
// phan quyen du an KIEU CU). Duoc GIU LAI nguyen ven o day chi de tuong
// thich nguoc (phong khi con giao dien/client cu nao goi den se khong bi
// loi 404) - hien tai da ra soat toan bo frontend trong repo
// (public_index.html, admin-users.html, progress-tasks.html,
// shim_frontend.js) va KHONG con noi nao goi 2 API nay nua. Quan trong: du
// gui du lieu len day, 2 API nay KHONG con anh huong gi den quyen thuc te
// cua ai ca - nguon quyen chinh thuc duy nhat la user_project_permissions
// (qua PUT /api/users/:userId/project-permissions) va user_task_permissions.
// Xem them chu thich o db.js (migrateLegacyMembersToPermissions).
...
// [HARDENING] Day la API DUY NHAT trong toan bo server co the THEM gia tri
// vao danh muc dung chung (categories) - da ra soat toan bo file server.js:
// khong co API GET/PUT/PATCH/DELETE nao khac thao tac ghi len bang
// categories. Chi Quan tri vien va Quan ly moi duoc phep them/sua danh muc;
// Nguoi phu trach va Nguoi xem KHONG duoc phep (bi chan 403 o day, ngay tai
// server - khong chi an nut tren giao dien). Nguoi phu trach/Nguoi xem van
// DOC va SU DUNG duoc cac gia tri danh muc hien co binh thuong khi nhap lieu
// du an/task, vi du nay chi chan thao tac GHI (them moi danh muc) - du lieu
// danh muc van duoc tra ve day du cho moi vai tro qua GET /api/bootstrap.
app.post("/api/categories/:cat", requireRole("admin", "manager"), (req, res) => {
